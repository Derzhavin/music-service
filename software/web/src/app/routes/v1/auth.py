...
from services import UsersSecurityService
...
async def login(login: Login, users_security_service: UsersSecurityService = Depends()):
    if not users_security_service.authenticate_user(login.username, login.password):
